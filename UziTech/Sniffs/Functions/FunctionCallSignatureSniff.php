...
namespace UziTech\Sniffs\Functions;
			...
			if ($fix === true) {
				...
				if ($this->tabIndent) {
					$phpcsFile->fixer->addContent(
						$openBracket,
						$phpcsFile->eolChar.str_repeat('\t', (($functionIndent / $this->indent) + 1))
					);
				} else {
				// XXX: added by Tony
					$phpcsFile->fixer->addContent(
			...
			if ($tokens[$i]['line'] !== $lastLine) {
				...
				if ($tokens[$i]['line'] !== $tokens[$openBracket]['line']) {
					...
					if ($foundIndent < $expectedIndent
						...
						// XXX: added by Tony
						if ($this->tabIndent) {
							$error = 'Multi-line function call not indented correctly; expected %s tabs but found %s';
							$data  = [
								(int) ($expectedIndent / $this->indent),
								(int) ($foundIndent / $this->indent),
							];
						} else {
						// XXX: added by Tony
						...
						if ($fix === true) {
							// XXX: added by Tony
							if ($this->tabIndent) {
								$padding = str_repeat('\t', $expectedIndent / $this->indent);
							} else {
							// XXX: added by Tony
