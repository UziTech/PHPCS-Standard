...
class UziTech_Sniffs_Arrays_ArrayDeclarationSniff implements PHP_CodeSniffer_Sniff
{
    ...
    public function processMultiLineArray(PHP_CodeSniffer_File $phpcsFile, $stackPtr, $arrayStart, $arrayEnd)
    {
        ...
        if ($tokens[$lastContent]['line'] === $tokens[$arrayEnd]['line']) {
            ...
            $error    = 'Closing parenthesis not aligned correctly; expected %s tab(s) but found %s';
            $data     = array(
                         $expected,
                         $found,
                        );
        ...
        if ($keyUsed === false && empty($indices) === false) {
            ...
            foreach ($indices as $value) {
                ...
                if ($lastValueLine !== false && $tokens[$value['value']]['line'] === $lastValueLine) {
                    ...
                    $found = (int)floor(($tokens[$first]['column'] - 1) / $this->indent);
                    if ($found !== $expected) {
                        $error = 'Array value not aligned correctly; expected %s tab(s) but found %s';
                        $data  = array(
                                  $expected,
                                  $found,
                                 );
