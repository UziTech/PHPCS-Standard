...
namespace UziTech\Sniffs\CodeAnalysis;

use UziTech\Lib\ScopeInfo;
use UziTech\Lib\VariableInfo;
use UziTech\Lib\Constants;
use UziTech\Lib\Helpers;
...
class VariableAnalysisSniff implements Sniff {
	 ...
	 *  @var string|null
	 */
	public $validUndefinedVariableNames = "user analytics";
	 ...
	 * @return void
	 */
	protected function addError($message, $stackPtr, $code, $data) {
		if (! $this->currentFile) {
	...
	protected function checkForFunctionPrototype(File $phpcsFile, $stackPtr, $varName, $currScope) {
		...
		if (($functionPtr !== false) && ($tokens[$functionPtr]['code'] === T_USE)) {
			...
			if ($this->isVariableUndefined($varName, $stackPtr, $currScope) === true) {
				...
				$phpcsFile->addError("Variable %s is undefined.", $stackPtr, 'UndefinedVariable', ["\${$varName}"]);
				return true;
			}
	...
	protected function processScopeCloseForVariable(File $phpcsFile, VariableInfo $varInfo, ScopeInfo $scopeInfo) {
		...
		} else if (!empty($varInfo->firstInitialized)) {
